...
class User(db.Model, UserMixin):
    ...
    client_id = db.Column(
        db.Integer, db.ForeignKey("client.id", name="fk_user_client"), nullable=True
    )
    client = db.relationship("Client", backref="user_account")
...
class Ticket(db.Model):
    ...
    assigned_to = db.Column(
        db.Integer,
        db.ForeignKey("user.id", name="fk_ticket_assigned_to"),
        nullable=True,
    )
    ...
    note_start_time = db.Column(
        db.DateTime, nullable=True
    )  # For tracking when a note starts
    ...
    phase_id = db.Column(
        db.Integer, db.ForeignKey("phase.id", name="fk_ticket_phase"), nullable=True
    )

    # Foreign key to Project (to link the ticket to a project)
    project_id = db.Column(
        db.Integer, db.ForeignKey("project.id", name="fk_ticket_project"), nullable=True
    )
    ...
    phase = db.relationship(
        "Phase", back_populates="tickets"
    )  # Back_populates instead of backref

    # Foreign key to the client (employee) who made the request
    client_id = db.Column(
        db.Integer, db.ForeignKey("client.id", name="fk_ticket_client"), nullable=False
    )
...
class TicketNote(db.Model):
    ...
    ticket_id = db.Column(
        db.Integer,
        db.ForeignKey("ticket.id", name="fk_ticketnote_ticket"),
        nullable=False,
    )
    user_id = db.Column(
        db.Integer, db.ForeignKey("user.id", name="fk_ticketnote_user"), nullable=True
    )
...
class Project(db.Model):
    ...
    company_id = db.Column(
        db.Integer,
        db.ForeignKey("company.id", name="fk_project_company"),
        nullable=False,
    )
...
class Phase(db.Model):
    ...
    project_id = db.Column(
        db.Integer, db.ForeignKey("project.id", name="fk_phase_project"), nullable=False
    )
...
class Client(db.Model):
    ...
    company_id = db.Column(
        db.Integer,
        db.ForeignKey("company.id", name="fk_client_company"),
        nullable=False,
    )
